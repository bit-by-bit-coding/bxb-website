...
document.addEventListener('DOMContentLoaded', function() {
  ...
  options = {
    algorithm: random_choice(Object.keys(algorithms)),
    pivot: 'Random',
    shrink_factor: 1.3,
    base: 2,
  ...
  options.width = Math.floor($(canvas).innerWidth() / options.zoom);
  options.height= Math.floor($(canvas).height() / options.zoom);
  ...
  hide_gui_element(gui, 'shrink/_factor', true);
